...
variable [dc : DecidableRel (lc.lt.lt)]

inductive Interp (i : lc.L) (I : lc.L → Term → Prop) : Term → (Term → Prop) → Prop where
...
theorem what {i} : (λ j a ↦ if j < i then (∃ P, Interps j a P) else False) = (λ j a ↦ ∃ P, ⟦ a ⟧ j ↘ P) := by
  funext j a
  apply propext
  constructor
  . intro h; split at h; assumption; contradiction
  . intro h; split; assumption; sorry

-- ⚠️ uses propext ⚠️
theorem unfoldInterps {a i P} :
  (⟦ a ⟧ i ↘ P) =
  (⟦ a ⟧ i , (λ j b ↦ ∃ P, ⟦ b ⟧ j ↘ P) ↘ P) := by
  apply propext; constructor
  case _ =>
    intro h; unfold Interps at h
    induction h
    case ne => apply Interp.ne; assumption
    case pi => apply Interp.pi; repeat assumption
    case 𝒰 => sorry -- the interpretation needs a proof of j < i to get rid of the ifte
    case mty => apply Interp.mty
    case lvl => apply Interp.lvl; assumption
    case step => apply Interp.step; repeat assumption
  case _ =>
    intro h; unfold Interps
    induction h
    case ne => apply Interp.ne; assumption
    case pi => apply Interp.pi; repeat assumption
    case 𝒰 => sorry -- the interpretation needs a proof of j < i to get rid of the ifte
...
theorem interp𝒰Eq {I : lc.L → Term → Prop} {b c} (r : b ⇒ c) :
  (λ a ↦ (∃ j, b ⇒⋆ lof j ∧ I j a) ∨ wne a) =
  ...
  . intro Pa; rcases Pa with ⟨j, rj, h⟩ | wnea
    ...
    . exact Or.inr wnea
  . intro Pa; rcases Pa with ⟨j, rj, h⟩ | wnea
    . exact Or.inl ⟨j, Pars.trans r rj, h⟩
    . exact Or.inr wnea
...
theorem interp𝒰Inv {i I b P} (h : ⟦ 𝒰 b ⟧ i , I ↘ P) :
  wnf b ∧ P = (λ a ↦ (∃ j, b ⇒⋆ lof j ∧ I j a) ∨ wne a) := by
...
theorem interps𝒰 {i b} (wnfb : wnf b) :
  ⟦ 𝒰 b ⟧ i ↘ (λ a ↦ (∃ j, b ⇒⋆ lof j ∧ ∃ P, ⟦ a ⟧ j ↘ P) ∨ wne a) := by
  rewrite [unfoldInterps] at *; constructor; assumption
...
theorem interpsBwdsP {i a x y P} (r : x ⇒⋆ y) (h : ⟦ a ⟧ i ↘ P) : P y → P x := by
  ...
  case 𝒰 =>
    intro Py; rcases Py with ⟨j, rj, h⟩ | wney
    . split at h
      . let ⟨P, h⟩ := h
        refine Or.inl ⟨j, rj, ?_⟩
        split; exact ⟨P, interpsBwds r h⟩; contradiction
      . contradiction
...
theorem interpsCumul {i j a P} (lt : i < j) (h : ⟦ a ⟧ i ↘ P) : ⟦ a ⟧ j ↘ P := by
  rewrite [unfoldInterps] at h; induction h generalizing j
  ...
  case 𝒰 k wnfk => apply interps𝒰 wnfk
...
theorem interps𝒰Inv {i b P} (h : ⟦ 𝒰 b ⟧ i ↘ P) :
  wnf b ∧ P = (λ a ↦ (∃ j, b ⇒⋆ lof j ∧ ∃ P, ⟦ a ⟧ j ↘ P) ∨ wne a) := by
  rewrite [unfoldInterps] at h; exact interp𝒰Inv h
...
theorem adequacy {i a P} (h : ⟦ a ⟧ i ↘ P) : CR P := by
  ...
  case 𝒰 j _ =>
    ...
    . exact Or.inr
    . intro h
      rcases h with ⟨j, _, h⟩ | wnfb
      . split at h <;> try contradiction
        let ⟨P, h⟩ := h
        rewrite [unfoldInterps] at h
        refine interpWnf (λ {a} {P} ha ↦ @adequacy j a P ?_) h
        rewrite [unfoldInterps]; exact ha
      . exact wneWnf wnfb
...
theorem interpsWnf {i a P} (h : ⟦ a ⟧ i ↘ P) : wnf a := by
  unfold Interps at h
  refine interpWnf (λ {a} {P} ha ↦ @adequacy _ _ i a P ?_) h

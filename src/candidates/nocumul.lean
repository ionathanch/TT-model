...
theorem transl {i j k : lc.L} (lt : i < j) (le : j ≤ k) : i < k := by
  rcases le with ltjk | e
  . exact IsTrans.trans i j k lt ltjk
  . subst e; exact lt

theorem transr {i j k : lc.L} (le : i ≤ j) (lt : j < k) : i < k := by
  rcases le with ltij | e
  . exact IsTrans.trans i j k ltij lt
  . subst e; exact lt

inductive Interp : ∀ (i : lc.L), (∀ j, j < i → Term → Prop) → Term → (Term → Prop) → Prop where
  | ne i I a : ne a → Interp i I a wne
  | pi i I j le a b Pa (Pf : Term → (Term → Prop) → Prop) :
    Interp j (λ k lt ↦ I k (transl lt le)) a Pa →
    (∀ x, Pa x → ∃ Pb, Pf x Pb) →
    (∀ x Pb, Pf x Pb → Interp j (λ k lt ↦ I k (transl lt le)) (subst (x +: var) b) Pb) →
    Interp i I (pi a b) (λ f ↦ ∀ x Pb, Pa x → Pf x Pb → Pb (app f x))
  | 𝒰 i I b : wnf b → Interp i I (𝒰 b)
    (λ a ↦ (∃ j lt, b ⇒⋆ lof j ∧ I j lt a) ∨ wne a)
  | mty i I : Interp i I mty wne
  | lvl i I b : wnf b → Interp i I (lvl b)
...
/-*-------------------------------------------------------
  Helper for interpretations of `𝒰` and `lvl`:
...
theorem interp𝒰Eq {i} {I : ∀ j, j < i → Term → Prop} {b c} (r : b ⇒ c) :
  ...
  . intro Pa; rcases Pa with ⟨j, lt, rj, h⟩ | wnea
    . let ⟨k', rk', r₂⟩ := diacon rj r
      rw [parsLofInv rk'] at r₂
      refine Or.inl ⟨j, lt, r₂, h⟩
    . exact Or.inr wnea
  . intro Pa; rcases Pa with ⟨j, lt, rj, h⟩ | wnea
    . exact Or.inl ⟨j, lt, Pars.trans r rj, h⟩
    . exact Or.inr wnea

theorem interp𝒰Lt {j k : lc.L} {b} (lt : j < k) : ∀ {a},
  (∃ i, ∃ (_ : i < j), b ⇒⋆ lof i ∧ ∃ P, ⟦ a ⟧ i ↘ P) ∨ wne a →
  (∃ i, ∃ (_ : i < k), b ⇒⋆ lof i ∧ ∃ P, ⟦ a ⟧ i ↘ P) ∨ wne a := by
  intro a Pa; rcases Pa with ⟨i, lt, r, h⟩ | wneb
  . refine Or.inl ⟨i, ?_, r, h⟩; apply IsTrans.trans; repeat assumption
  . exact Or.inr wneb
...
theorem interpPiInv {i I a b P} (h : ⟦ pi a b ⟧ i , I ↘ P) :
  ∃ j le, ∃ (Pa : Term → Prop) (Pf : Term → (Term → Prop) → Prop),
    (⟦ a ⟧ j , (λ k lt ↦ I k (transl lt le)) ↘ Pa) ∧
    (∀ x, Pa x → ∃ Pb, Pf x Pb) ∧
    (∀ x Pb, Pf x Pb → ⟦ subst (x +: var) b ⟧ j, (λ k lt ↦ I k (transl lt le)) ↘ Pb) ∧
    P = (λ f ↦ ∀ x Pb, Pa x → Pf x Pb → Pb (app f x)) := by
  generalize e : pi a b = c at h
  induction h generalizing a b
  case ne => subst e; contradiction
  case pi lt _ _ Pa Pf ha hPf hb _ _ =>
    injection e with ea eb; subst ea; subst eb
    exact ⟨_, lt, Pa, Pf, ha, hPf, hb, rfl⟩
  case step r _ ih =>
    subst e; cases r
    let ⟨_, le, Pa, Pf, ha, hPf, hb, e⟩ := ih rfl
    refine ⟨_, le, Pa, Pf, ?_, hPf, ?_, e⟩
...
theorem interp𝒰Inv {i I b P} (h : ⟦ 𝒰 b ⟧ i , I ↘ P) :
  ...
  case ne => subst e; contradiction
  case 𝒰 wnfb => injection e with e; subst e; exact ⟨wnfb, rfl⟩
  case step r _ ih =>
    subst e; let (Par.𝒰 r₁) := r
    let ⟨wnfb, e⟩ := ih rfl
    rw [interp𝒰Eq r₁]
    exact ⟨wnfBwds (parPars r₁) wnfb, e⟩
  all_goals contradiction
...
theorem interpLvlInv {i I b P} (h : ⟦ lvl b ⟧ i , I ↘ P) :
  ...
  case step r _ ih =>
    ...
    let ⟨wnfc, e⟩ := ih rfl
    rw [interpLvlEq r₁]
    exact ⟨wnfBwds (parPars r₁) wnfc, e⟩
  all_goals contradiction
...
theorem interpsPi {i j a b Pa P}
  (le : j ≤ i) (ha : ⟦ a ⟧ j ↘ Pa)
  (hb : ∀ x, Pa x → ∃ Pb, ⟦ subst (x +: var) b ⟧ j ↘ Pb) :
  P = (λ f ↦ ∀ x Pb, Pa x → (⟦ subst (x +: var) b⟧ j ↘ Pb) → Pb (app f x)) →
  ⟦ pi a b ⟧ i ↘ P := by
  unfold Interps at *; intro e; subst e; constructor
  all_goals first | simp | assumption

theorem interps𝒰 {i b} (wnfb : wnf b) :
  ⟦ 𝒰 b ⟧ i ↘ (λ a ↦ (∃ j, ∃ (_ : j < i), b ⇒⋆ lof j ∧ ∃ P, ⟦ a ⟧ j ↘ P) ∨ wne a) := by
  unfold Interps at *; constructor; assumption

theorem interpsMty {i} : ⟦ mty ⟧ i ↘ wne := by
  unfold Interps at *; apply Interp.mty
...
theorem interpFwd {i I a b P} (r : a ⇒ b) (h : ⟦ a ⟧ i , I ↘ P) : ⟦ b ⟧ i , I ↘ P := by
  ...
  case 𝒰 => cases r; case 𝒰 wnfb _ r =>
    rw [interp𝒰Eq r]; constructor; exact wnfFwds (parPars r) wnfb
  case mty => cases r; apply Interp.mty
...
theorem interpsBwdsP {i a x y P} (r : x ⇒⋆ y) (h : ⟦ a ⟧ i ↘ P) : P y → P x := by
  unfold Interps at h
  generalize e : (λ j _ a ↦ ∃ P, ⟦ a ⟧ j ↘ P) = I at h
  induction h generalizing x y <;> subst e
  case ne => exact wneBwds r
  case pi ihb =>
    intro h x Pb Pax PfxPb
    exact ihb x Pb PfxPb (parsApp r (Pars.refl x)) rfl (h x Pb Pax PfxPb)
  case 𝒰 =>
    intro Py; rcases Py with ⟨j, lt, rj, h⟩ | wney
    . let ⟨P, h⟩ := h; exact Or.inl ⟨j, lt, rj, ⟨P, interpsBwds r h⟩⟩
  ...
  case lvl =>
    ...
    . exact Or.inr (wneBwds r wney)
  case step ih => exact ih r rfl

/-*--------------------------------
  Interpretation is deterministic
--------------------------------*-/

theorem interpsCumul {i j a P} (lt : i < j) (h : ⟦ a ⟧ i ↘ P) :
  ∃ Q, (∀ {b}, P b → Q b) ∧ ⟦ a ⟧ j ↘ Q := by
  unfold Interps at h
  generalize e : (λ k _ a ↦ ∃ R, ⟦ a ⟧ k ↘ R) = I at h
  induction h generalizing j <;> subst e
  case ne nea => exact ⟨wne, id, interpsNe nea⟩
  case pi =>
    unfold Interps; refine ⟨_, id, ?_⟩
    apply Interp.pi (le := Or.inl (transr ?_ lt))
    all_goals assumption
  case 𝒰 k wnfk => exact ⟨_, interp𝒰Lt lt, interps𝒰 wnfk⟩
  case mty => exact ⟨wne, id, interpsMty⟩
  case lvl wnf => exact ⟨_, id, interpsLvl wnf⟩
  case step r _ ih =>
    let ⟨Q, PQ, h⟩ := ih lt rfl
    exact ⟨Q, PQ, interpsBwd r h⟩

theorem interpsCumul' {i j a P Q} (le : i ≤ j) (hP : ⟦ a ⟧ i ↘ P) (hQ : ⟦ a ⟧ j ↘ Q) :
  ∀ {b}, P b → Q b := by
  unfold Interps at hP
  generalize e : (λ k _ a ↦ ∃ R, ⟦ a ⟧ k ↘ R) = I at hP
  induction hP generalizing j Q <;> subst e
  case pi jP leP _ _ Pa Pf _ hPf _ iha ihb =>
    unfold Interps at hQ
    let ⟨jQ, leQ, Pa', Pf', ha', hPf', hb', e⟩ := interpPiInv hQ
    subst e
    intro f PQ x Pb Pax PfxPb
    sorry
  all_goals sorry

-- ⚠️ uses funext and propext ⚠️
theorem interpDet' {i I a P Q}
  (hP : ⟦ a ⟧ i , I ↘ P)
  (hQ : ⟦ a ⟧ i , I ↘ Q) : P = Q := by
  induction hP generalizing Q
  case ne nea => exact symm (interpNeInv hQ nea)
  case pi Pa Pf _ hPf _ iha ihb =>
    let ⟨_, le, Pa', Pf', ha', hPf', hb', e⟩ := interpPiInv hQ
...
theorem interps𝒰Inv {i b P} (h : ⟦ 𝒰 b ⟧ i ↘ P) :
  wnf b ∧ P = (λ a ↦ (∃ j, ∃ (_ : j < i), b ⇒⋆ lof j ∧ ∃ P, ⟦ a ⟧ j ↘ P) ∨ wne a) := by
  unfold Interps at h; exact interp𝒰Inv h
...
theorem adequacy {i a P} (h : ⟦ a ⟧ i ↘ P) : CR P := by
  ...
  case 𝒰 j _ =>
    ...
    . exact Or.inr
    . intro h
      rcases h with ⟨j, _, _, _, h⟩ | wnfb
      . unfold Interps at h
        refine interpWnf (λ {a} {P} ha ↦ @adequacy j a P ?_) h

...
theorem renameComp' ξ ζ ς (h : ∀ x, (ξ ∘ ζ) x = ς x) : ∀ s, (rename ξ ∘ rename ζ) s = rename ς s := by
  intro s; revert ξ ζ ς h
  induction s <;> intro ξ ζ ς h <;> simp <;> try constructor
  case var s => rw [← h]; rfl
...
theorem substExt σ τ (h : ∀ x, σ x = τ x) : ∀ s, subst σ s = subst τ s := by
  intro s; revert σ τ h
  induction s <;> intro σ τ h <;> simp <;> try constructor
  case var => rw [h]
  case pi.left ih _ => apply ih; assumption
...
theorem substId' σ (h : ∀ x, σ x = var x) : ∀ s, subst σ s = s := by
  intro s; revert σ h
  induction s <;> intro σ h <;> simp <;> try constructor
  case var => rw [h]
  case pi.left ih _ => apply ih; assumption
...
theorem substRename' ξ σ τ (h : ∀ x, (σ ∘ ξ) x = τ x) : ∀ s, subst σ (rename ξ s) = subst τ s := by
  intro s; revert ξ σ τ h
  induction s <;> intro ξ σ τ h <;> simp <;> try constructor
  case var => rw [← h]; rfl
  case pi.left ih _ => apply ih; assumption

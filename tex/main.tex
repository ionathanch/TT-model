...
Const ℓ₁ ℓ₂ A B = A → B → A
  ...
  \lang &
  \begin{minipage}[t]{0.86\textwidth}
  $\kw{Const} :    \Pi  l  \mathbin{:}   \kw{Level}\texttt{<} \gap   \omega    \mathpunct{.}   \Pi  j  \mathbin{:}   \kw{Level}\texttt{<} \gap  l   \mathpunct{.}   \Pi  k  \mathbin{:}   \kw{Level}\texttt{<} \gap  l   \mathpunct{.}   \kw{U} \gap  j      \to   \kw{U} \gap  \ottnt{k}    \to   \kw{U} \gap  \ell  $ \\
  \phantom{$\kw{Const}$} $\coloneq    \lambda  l  \mathpunct{.}   \lambda  j  \mathpunct{.}   \lambda  k  \mathpunct{.}   \lambda  A  \mathpunct{.}   \lambda  B  \mathpunct{.}  A       \to  \ottnt{B}   \to  \ottnt{A} $
  \end{minipage}
\end{tabular}
\end{table}
...
Their typing rules are straightforward, given below. % in \cref{fig:lvl-ops}.
%
% \begin{figure}
\begin{mathpar}
...
\end{mathpar}
% \caption{Typing rules for level operators}
% \label{fig:lvl-ops}
% \end{figure}
...
A number of these subtyping rules are given below, % in \cref{fig:subtyping},

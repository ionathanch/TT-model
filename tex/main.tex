...
]{Software}{https://github.com/ionathanch/TTBFL}
...
if the type of a universe is itself, all types are inhabited.
Therefore, in Martin-L\"of's type theory (MLTT)~\citep{mltt},
he stratified the universe into a countably infinite hierarchy of universes
...
\begin{itemize}[topsep=0pt]
  ...
  \item We prove subject reduction (\ie preservation),
  ...
  \item Using a syntactic logical relation,
    ...
    Normalization remains an open problem.
\end{itemize}
%
...
we additionally use dependent pair types, equality types, and list types
...
\begin{align*}
  ...
  &\phantom{{} \coloneqq {}}   \Pi  f  \mathbin{:}   (   \Pi  x  \mathbin{:}  A  \mathpunct{.}  B   \gap  x  )   \mathpunct{.}   \Pi  g  \mathbin{:}   (   \Pi  x  \mathbin{:}  A  \mathpunct{.}  B   \gap  x  )   \mathpunct{.}   (     \Pi  x  \mathbin{:}  A  \mathpunct{.}  f   \gap  x   =  g   \gap  x  )     \to    \ottmv{f}  =  \ottmv{g}   
\end{align*}
...
\begin{align*}
  ...
  &\phantom{{} : {}}   (       \Pi  x  \mathbin{:}  A  \mathpunct{.}   \Pi  y  \mathbin{:}   B  \gap  x   \mathpunct{.}  f    \gap  x   \gap  y   =  g   \gap  x   \gap  y  )   \to    \ottmv{f}  =  \ottmv{g}   
\end{align*}
...
id ℓ A a = a
  ...
  \\
  \midrule
  \lang & $\kw{id} :   \Pi  k  \mathbin{:}   \kw{Level}\texttt{<} \gap   \omega    \mathpunct{.}   \Pi  A  \mathbin{:}   \kw{U} \gap  k   \mathpunct{.}  A    \to  A  \coloneq  \lambda  k  \mathpunct{.}   \lambda  A  \mathpunct{.}   \lambda  a  \mathpunct{.}  a   $
\end{tabular}
\end{table}
...
Id = ∀ (ℓ : Level) (A : Set ℓ) → A → A
  ...
  \\
  \midrule
  \lang & $\kw{Id} :  \kw{U} \gap   \omega   \coloneq   \Pi  k  \mathbin{:}   \kw{Level}\texttt{<} \gap   \omega    \mathpunct{.}   \Pi  A  \mathbin{:}   \kw{U} \gap  k   \mathpunct{.}  A    \to  A $
\end{tabular}
\end{table}
...
Const ℓ₁ ℓ₂ = ∀ (A : Set ℓ₁) (B : Set ℓ₂) → A → B → A
  ...
  \phantom{$\kw{Const}$} $\coloneq    \lambda  l  \mathpunct{.}   \lambda  j  \mathpunct{.}   \lambda  k  \mathpunct{.}   \Pi  A  \mathbin{:}   \kw{U} \gap  j   \mathpunct{.}   \Pi  B  \mathbin{:}   \kw{U} \gap  k   \mathpunct{.}  A       \to  B   \to  A $
  \end{minipage}
\end{tabular}
\end{table}
...
$  \ottmv{f}  \mathbin{:}    \kw{U} \gap   2    \to   \kw{U} \gap   0      \vdash    \lambda  \ottmv{x}  \mathbin{:}   \kw{U} \gap   1    \mathpunct{.}  \ottmv{f}   \gap  \ottmv{x}   \mathrel{:}    \kw{U} \gap   1    \to   \kw{U} \gap   1    $.
...
The theorem is stated in terms of a call-by-name evaluation strategy,
...
defined as follows.
\begin{equation*}
  \ottnt{v} \Coloneqq \ottmv{i} \mid  \Pi  \ottmv{x}  \mathbin{:}  \ottnt{A}  \mathpunct{.}  \ottnt{B}  \mid  \lambda  \ottmv{x}  \mathbin{:}  \ottnt{A}  \mathpunct{.}  \ottnt{b}  \mid  \bot  \mid  \kw{U} \gap  \ottnt{k}  \mid  \kw{Level}\texttt{<} \gap  \ell 
\end{equation*}
...
Complete development \fbox{$ \ottnt{a} ^{ \mathsf{T} } $} is defined in \cref{fig:taka},
...
we prove the usual weakening, substitution, replacement, and regularity lemmas.
...
\begin{lemma}[Context well-typedness] \label{lem:wt:ctxt}
...
\begin{corollary}[\textsc{Lam'}] \label{Lam'}
...
\subsection{Curry-style violates normalization} \label{sec:paradox}
...
While we have not found other counterexamples to normalization in our Church-style presentation,
it is also unclear how to define extend the logical relation to prove normalization
...
We have proven our type theory to be type safe and logically consistent.
...
Whether the normalization issues discussed in \cref{sec:paradox}
are inherent to the system is unclear.
The only formal syntactic system we know of
with universe level polymorphism beyond prenex polymorphism
is BCDE~\citep{univ-poly}.
However, their system does not satisfy subject reduction,
and their proposed amendment to allow looping level constraints of the form $\ottnt{k} < \ottnt{k}$
to fix the issue would permit type-in-type in a looping context
and break normalization.
Even so, we are hopeful that normalization holds,

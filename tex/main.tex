...
At this point, talking about simultaneous substitutions is inevitable.
...
is defined in terms of semantic typing,
but it is more convenient use the admissible rules defined in \cref{fig:sem:subst}.
...
\begin{mathpar}
  ...
  \inferrule[\ottdrulename{S-Nil}]{~}{ \sigma  \vDash   \cdot  }
  \and
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item \textit{\Rref{Level<}.}
      ...
      $ U  \gap   (  \ottnt{k_{{\mathrm{1}}}} [  \sigma  ]  )  $ is in the interpretation of a universe.
      By inversion, we know that it too has an interpretation as a universe
      such that our remaining goal is now $ \mathopen{\llbracket}   \kw{Level}\texttt{<} \gap   (  \ottnt{k_{{\mathrm{0}}}} [  \sigma  ]  )    \mathclose{\rrbracket}_{ \ottmv{j} } \searrow  \ottnt{P} $ for some $\ottnt{P}$,

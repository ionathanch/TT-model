...
solely knowing that $  \cdot   \vdash   2   \mathrel{:}   \kw{Level}\texttt{<} \gap   3   $, $  \cdot   \vdash   \kw{U} \gap   0    \mathrel{:}   \kw{U} \gap   1   $,
...
\begin{proof} ~
  \begin{enumerate}[topsep=0pt]
    ...
    \item For $ \ottnt{A}  \Rightarrow  \ottnt{B} $, directly by \rref{I-Step}.
      For $ \ottnt{A}  \Rightarrow^\ast  \ottnt{B} $, by induction on this reduction. \qedhere
  \end{enumerate}
\end{proof}
...
then inversion on \rref{I-Pi} gives \emph{some} interpretation $\ottnt{P_{{\mathrm{2}}}}$ of the codomain,
...
\begin{definition} \thmref{semantics.lean}{semSubst}
  ...
  wrt context $\Gamma$ iff for every $ \ottmv{x}  \mathrel{:}  \ottnt{A}  \in  \Gamma $,
...
\begin{corollary}[Canonicity of types] \thmref{soundness.lean}{canon$\mathcal{U}$} \label{lem:canon:univ}
  ...
  then $ \ottnt{C}  \Rightarrow^\ast   \Pi  \ottmv{x}  \mathbin{:}  \ottnt{A}  \mathpunct{.}  \ottnt{B}  $, $ \ottnt{C}  \Rightarrow^\ast   \kw{U} \gap   \ottmv{i}   $, $ \ottnt{C}  \Rightarrow^\ast   \kw{Level}\texttt{<} \gap   \ottmv{i}   $, or $ \ottnt{C}  \Rightarrow^\ast   \bot  $.
  By \nameref{thm:soundness},
  instantiating with the identity substitution,
